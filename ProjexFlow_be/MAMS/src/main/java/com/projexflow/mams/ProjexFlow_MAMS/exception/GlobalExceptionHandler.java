...
import com.projexflow.mams.ProjexFlow_MAMS.dto.ApiErrorResponse;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleGroupingNotCompleted(GroupingNotCompletedException ex, HttpServletRequest req) {
        return build(HttpStatus.CONFLICT, "GROUPING_NOT_CLOSED", ex.getMessage(),
                "Grouping is still OPEN. Mentor assignment is available after the admin CLOSES grouping.",
                req, null);
    }
    ...
    public ResponseEntity<ApiErrorResponse> handleConflict(ConflictException ex, HttpServletRequest req) {
        return build(HttpStatus.CONFLICT, "CONFLICT", ex.getMessage(), ex.getMessage(), req, null);
    }
